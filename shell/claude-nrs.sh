...
tmux send-keys -t "Main:${WIN}" 'pkexec nixos-rebuild switch --flake ~/nixos-config#lap-nix && notify-send "NixOS Rebuild" "Switch successful" && exit || notify-send -u critical "NixOS Rebuild" "Switch failed"' Enter
