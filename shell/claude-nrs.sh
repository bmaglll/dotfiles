...
tmux send-keys -t "Main:${WIN}" 'if pkexec nixos-rebuild switch --flake ~/nixos-config#lap-nix; then notify-send -i ~/nixos-config/icons/NixOS.svg "NixOS Rebuild" "Switch successful"; mpv --no-video ~/nixos-config/sounds/nrs_notification.mp3 &; exit; else notify-send -u critical -i ~/nixos-config/icons/NixOS.svg "NixOS Rebuild" "Switch failed"; mpv --no-video ~/nixos-config/sounds/nrs_notification.mp3 &; fi' Enter
